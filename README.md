...
```
飞书缺陷 → bug_analysis（拉取详情+附件+分析日志/源码+回写结论）
                                                         ↑
...
| Git + GitHub SSH Key | 克隆/更新 11 个 C++ 参考源码仓库 |
...
# ferrit 和 ov580_driver 含 submodule
cd nreal-ferrit && git submodule update --init --recursive && cd ..
cd nreal-ov580_driver && git submodule update --init --recursive && cd ..
# 工具库 / 编解码 / 构建仓库
git clone --branch develop git@github.com:nreal-ai/nrealUtil.git
git clone --branch develop git@github.com:nreal-ai/xr_codec.git nreal-xr_codec
git clone --branch master git@github.com:nreal-ai/nrsdkrepo.git nreal-nrsdkrepo
cd ..
```
