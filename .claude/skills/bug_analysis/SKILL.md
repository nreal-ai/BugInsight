...
眼镜端日志目录（`current_log_dir/`）下通常包含以下文件，**分析时必须全部检查**：

| 日志文件 | 内容 | 必查 |
|---------|------|------|
| `pilot.log` | Pilot 主进程日志（Flinger/Display/DP 管线） | ✅ |
| `user.log` | syslog user facility — **用户态 daemon/app 日志**（Leopard、Ferrit 等模块的 ERROR/FATAL 均在此） | ✅ |
| `kernel.log` | syslog kernel facility — Linux kernel 日志（USB/DP Alt Mode/驱动） | ✅ |
| `mpp.log` | MPP/媒体处理管线日志 | 按需 |

**关键认知**：`user.log` 和 `pilot.log` 都在眼镜端同一目录下，都是眼镜端的日志。`user.log` 包含 Leopard（感知模块）、Ferrit（OSD 模块）等 daemon 进程的日志，gRPC 通信错误（如 `BaseBidiStreamReactor::OnReadDone`）、FrameService 断链等关键错误都记录在此。

**分析 checklist**：
1. 对 `pilot.log` 和 `user.log` **分别 grep ERROR 和 FATAL** 级别日志
2. 两个日志文件的错误必须交叉关联分析（时间戳对齐）
3. 不要只分析 `pilot.log` 而忽略 `user.log`
