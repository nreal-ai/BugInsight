...
"""
飞书缺陷全量数据获取脚本
使用 MCP JSON-RPC OFFSET 分页获取所有 ID，然后通过 Direct API 批量获取详情。
"""
...
def fetch_all_bug_ids():
    """通过 MCP JSON-RPC OFFSET 分页获取所有缺陷 ID（替代 mcporter CLI）"""
    from mcp_client import mcp_call, parse_moql_field_list
    ...
    # 先用 LIMIT 1 获取总数
    result, err = mcp_call("search_by_mql",
        {"project_key": PROJECT_KEY, "mql": f"SELECT work_item_id FROM {pk_ref}.issue LIMIT 1"},
        timeout=30)
    total = 0
    if result:
        total = result.get('list', [{}])[0].get('count', 0) if isinstance(result, dict) else 0
    print(f"  缺陷总数: {total}")
    ...
    for offset in range(0, total, batch_size):
        result, err = mcp_call("search_by_mql",
            {"project_key": PROJECT_KEY,
             "mql": f"SELECT work_item_id, name FROM {pk_ref}.issue LIMIT {batch_size} OFFSET {offset}"},
            timeout=30)
        if err:
            print(f"  批次 offset={offset} 失败: {err}")
            time.sleep(0.3)
            continue
        try:
            items = result.get('data', {}).get('1', []) if isinstance(result, dict) else []
            for item in items:
                moql_fields = parse_moql_field_list(item.get('moql_field_list', []))
                bug_id = moql_fields.get('work_item_id', '')
                bug_name = moql_fields.get('name', '')
                if bug_id:
                    all_bugs.append({'id': bug_id, 'name': bug_name, 'status': ''})

            print(f"  进度: {offset + len(items)}/{total}")
        except Exception as e:
            print(f"  批次 offset={offset} 解析失败: {e}")
...
def fetch_comments_via_mcporter(bug_ids, workers=5):
    """批量获取评论 via MCP JSON-RPC（替代 mcporter CLI）"""
    ...
    def check_comments(bug_id):
        try:
            result, err = mcp_call("list_workitem_comments",
                {"project_key": PROJECT_KEY, "work_item_id": str(bug_id), "page_num": 1},
                timeout=15)
            if result and isinstance(result, dict) and result.get('comments'):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            time.sleep(0.1)  # rate limiting per thread
