...
```markdown
## 🔍 AI分析结论 (by Claude Code + {模型名称})
```

模型名称从 `config.yaml` 的 `llm.model` 读取。末尾必须添加来源声明：

```markdown
> ⚠️ 此分析来源于 AI（Claude Code + {模型名称}），仅供参考。
```
...
每份报告必须包含置信度评估章节，位于根因分析之后、日志证据之前。使用**百分比制**统一评分：
...
| 维度 | 得分 | 说明 |
|------|------|------|
| 日志完整性 | 90% | user.log + pilot.log + kernel.log 三层完整覆盖 |
| 错误明确性 | 100% | DisplayService 明确报错 "unsupported edid(47)" |
| 根因确定性 | 93% | 完整调用链可追溯 control_display.cc:1176-1194 |

**综合：94% 🟢 高置信度**
```

维度说明：
- **日志完整性**：日志是否覆盖问题发生时段、包含关键进程日志（0-100%）
- **错误明确性**：错误码/异常类型是否明确指向具体组件（0-100%）
- **根因确定性**：代码分析是否能定位到具体函数/行号（0-100%）
- 综合得分 = 各维度加权平均，四舍五入取整
- 综合等级：🟢 高 (≥80%) | 🟡 中 (60%-79%) | 🔴 低 (<60%)
