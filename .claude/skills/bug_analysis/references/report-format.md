...
每份报告必须包含置信度评估章节，位于根因分析之后、日志证据之前：
...
维度说明：
- **日志完整性** (0.10)：日志是否覆盖问题发生时段、包含关键进程日志
- **错误明确性** (0.10)：错误码/异常类型是否明确指向具体组件
- **根因确定性** (0.15)：代码分析是否能定位到具体函数/行号
- 综合得分 = 各维度得分之和，满分 0.35
- 综合等级：🟢 高 (≥0.30) | 🟡 中 (0.20-0.29) | 🔴 低 (<0.20)
